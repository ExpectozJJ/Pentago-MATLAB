function [pos_played,rotation_played] = U1540273K_make_a_move_computerHard(Board,who_to_play)
...
% the opponent's 4 in a row, column of diagonal sequence so is for 3 in a
% row,column and diagonal. 
[criticalmove,centremoves] = BasicGenius(Board,who_to_play);
winningmoves = forthewin(Board,who_to_play);
defendbeflate = defendordie(Board,who_to_play);
            ...
            elseif win==0 % If there appears to be no winning state yet, it continues to run for loop for opponent's rotation move.
                for a=1:8 
                    ...
                    elseif win==0
                        ...
                        U1540273K_make_a_move_computerHard(boardai,who_to_play) % Recursives function
                    end
                end
            end
        end
...
elseif count==3 % If depth is 3 
    if numel(max2)~=0 % Check if there is a 2nd best move. 
        pos_played=commove1(randi([1,length(commove1)])); % Play a move from randomly chose average move list. 
        while Board(pos_played)~=0 % Check if the move is an invalid move 
            pos_played=commove1(randi([1,length(commove1)])); % Re-generate a different sverage move. 
        end
        if isempty(goodrot1) % If there is no good rotation moves. 
            rotation_played=comrot1(randi([1,length(comrot1)])); % Generate one average rotation from average rotation list. 
        else 
            rotation_played=goodrot1(randi([1,length(goodrot1)])); % Generate one good rotation move from the good rotation list. 
        end     
    end
elseif count==2 % If depth is 2 
    if numel(defense)~=0 % If there are defensive moves available. 
        pos_played=defense(randi([1,length(defense)])); % Randomly generate a defensive move from its list 
        while Board(pos_played)~=0 % Check if it is an invalid move 
           pos_played=defense(randi([1,length(defense)])); % Re-generate move 
        end
    else
        pos_played=commove1(randi([1,length(commove1)])); % Randomly generate an average move from its list. 
        while Board(pos_played)~=0 % Check if it is an invalid move
            pos_played=commove1(randi([1,length(commove1)])); % Re-generate move from its list again. 
        end
        if isempty(goodrot1) % If there is no good rotation moves. 
            rotation_played=comrot1(randi([1,length(comrot1)])); % Randomly generate an average rotation from its list. 
        else
            rotation_played=goodrot1(randi([1,length(goodrot1)])); % Randomly generate a good rotation from its list. 
        end
        count=1; % Resets depth to 1
    end
end

if numel(winningmoves)~=0 % If there are winning moves discovered
    pos_played=winningmoves(randi([1,length(winningmoves)])); % Generate a winning move from the list. 
    while Board(pos_played)~=0 % Check if it is an invalid move. 
        pos_played=winningmoves(randi([1,length(winningmoves)])); % Re-generate a winning move from the list. 
    end
    clear [pos_played,rotation_played] = U1540273K_make_a_move_computer(Board,who_to_play) % clear function 
elseif numel(criticalmove)~=0 % If there are critical moves discovered
    pos_played=criticalmove(randi([1,length(criticalmove)])); % Generate a critical move from the list. 
    while Board(pos_played)~=0 % Check if it is an invalid move. 
        pos_played=criticalmove(randi([1,length(criticalmove)])); % Re-generate a critical move from the list. 
    end
    clear [pos_played,rotation_played] = U1540273K_make_a_move_computer(Board,who_to_play) % clear function 
elseif numel(defendbeflate)~=0 % Check if there is a position to block 3 in a row, column or diagonal to become 4. 
    pos_played=defendbeflate(randi([1,length(defendbeflate)])); % Generates the move from the list of values in the vectors. 
    while Board(pos_played)~=0 % Check if it is an invalid move. 
        pos_played=defendbeflate(randi([1,length(defendbeflate)])); % Re-generate move.
    end
    clear [pos_played,rotation_played] = U1540273K_make_a_move_computer(Board,who_to_play) % clear function 
elseif numel(centremoves)~=0 % If centre tiles 8,11,26,29 (linear indexing) are not filled. 
    pos_played=centremoves(randi([1,length(centremoves)])); % Generates a move to make it on the centre move. 
    while Board(pos_played)~=0 % Check if the centre move to be made is invalid.
         pos_played=centremoves(randi([1,length(centremoves)])); % Re-generates again if need be. 
    end
    clear [pos_played,rotation_played] = U1540273K_make_a_move_computer(Board,who_to_play) % clear function
else
    clear [pos_played,rotation_played] = U1540273K_make_a_move_computer(Board,who_to_play) % clear function 
